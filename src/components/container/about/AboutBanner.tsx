import React from "react";

const AboutBanner = () => {
  return (
    <div className="banner-area pd-bottom-120 banner-small-inner bg-light bg-relative bg-cover">
      <div className="container">
        <h4>More About us.</h4>
        <h3>
          Specialized agency delivering advanced digital marketing, IT
          outsourcing, and data analytics solutions.
        </h3>
        <div className="row">
          <div className="col-lg-12 mt-lg-4 pt-lg-2">
            <p style={{ fontSize: "1.2rem" }}>
              Our specialized agency is dedicated to delivering cutting-edge
              solutions in digital marketing, IT outsourcing, and data
              analytics. With a deep understanding of the digital landscape, we
              harness the power of technology to help businesses achieve their
              goals and stay ahead in today's competitive market. Our digital
              marketing services encompass a wide range of strategies, from SEO
              and social media management to content creation and paid
              advertising, tailored to drive traffic and increase conversions.
              In IT outsourcing, we provide scalable and cost-effective
              solutions, leveraging our expertise in software development, cloud
              services, and infrastructure management. Additionally, our data
              analytics capabilities enable businesses to make informed
              decisions by leveraging data-driven insights for better
              performance and strategic planning. With a focus on innovation and
              client satisfaction, we are committed to delivering measurable
              results and driving business growth for our partners.
            </p>
          </div>
          {/* <div className="col-lg-6 order-lg-first text-lg-center">
            <div className="scroll-down top_image_bounce">
              <a href="#about-area">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  xmlnsXlink="http://www.w3.org/1999/xlink"
                  width="40"
                  height="80"
                  viewBox="0 0 40 80"
                >
                  <image
                    id="scroll"
                    width="40"
                    height="80"
                    xlinkHref="data:img/png;base64,"
                  />
                </svg>
              </a>
            </div>
          </div> */}
        </div>
      </div>
    </div>
  );
};

export default AboutBanner;
